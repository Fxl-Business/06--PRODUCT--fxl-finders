...
Red 2: after pnpm config migration, `printf '1\n' | make` failed with missing `@fxl-finders/shared-utils/dist/index.js`.
...
Local recheck after fix: `pnpm install --frozen-lockfile`, `pnpm run lint`, `pnpm run type-check`, `pnpm test`, `pnpm --filter @fxl-finders/api test:integration`, and `pnpm run perf:audit` exited 0.
