...
2. `printf '1\n' | make` built `@fxl-finders/shared-types` and `@fxl-finders/shared-utils` with `tsc`.
It then started `@fxl-finders/api` and printed `[fxl-finders-api] listening on http://localhost:3006 (development)`.

3. While the watcher was running, `curl -fsS -w '\nHTTP_STATUS:%{http_code}\n' http://localhost:3006/health` exited 0.
It returned JSON: `{"ok":true,"service":"fxl-finders-api","env":"development","version":"1.0.0","timestamp":"2026-07-05T00:45:03.278Z"}`.
...
`pnpm --filter @fxl-finders/api test:integration` exited 0, with 7 test files and 26 tests passing.
