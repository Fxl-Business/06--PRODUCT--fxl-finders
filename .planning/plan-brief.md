...
## Wave 4 execution outcomes (Phase 05 — conversion ingestion + commission ledger + audit)

14/14 tasks. verify PASS (28/28). code-review PASS (1 Critical found+fixed, 0 Warnings, 2 Info). **149 tests** (api unit 108 · shared-utils 17 · web 8 · integration 16). Migration `0004_huge_warbird` journaled. Live E2E webhook smoke green vs Postgres:5006.

**Exports now LIVE (Phase 06 consumes — do NOT recreate):**
- `apps/api/src/db/schema.ts`: `conversions`, `commissions`, `payouts` (D-Q single payouts table; `commissions.paid_payout_id` FK → payouts). All money int cents; `commissions.rate_pct` numeric(5,2). conversions/commissions `org_id NOT NULL` + FORCE RLS split-INSERT (D10); payouts NO RLS.
- `apps/api/src/domains/conversions/service.ts`: `ingestConversion(db, body, rawBodyHash)`, `WebhookBodySchema` (D-M ONE field set — **Phase 06 sender must byte-match this**), `RefundBodySchema`, `buildIdempotencyKey(source,orderId,eventType)` (plain sha256, D-N — **Phase 06 T04 byte-matches this**), `resolveAttribution`, `hashCustomerEmail`. **ingestConversion runs on getAdminDb() (BYPASSRLS) — webhook is cross-tenant, must read clicks/finders without a JWT.**
- `apps/api/src/domains/conversions/hmac-middleware.ts`: `hmacVerifyMiddleware` (D-O raw-body verify before parse via `c.req.raw.clone().arrayBuffer()`; generic 401 for ALL failure modes; reuses `verifyHmac` from shared-utils; payload `ts + "." + rawBody`).
- `apps/api/src/domains/commissions/service.ts`: `calculateSetupCommission`/`calculateRecurringCommission` (accept `number|string` rate — Drizzle numeric returns STRING), `isValidTransition` (D-K), `buildCommissionRows`, `getCommissionsByFinder` (getDb+setTenantContext), `getCommissionsAdmin`/`lockCommission`/`promoteHoldExpired`/`reverseCommission` (getAdminDb, audit per mutation). NO `approveCommission` (D-K). `commissionsRouter` (finder GET) + `commissionsAdminRouter` (GET, POST /promote-locked, /:id/lock, /:id/reverse).
- `apps/api/src/domains/payouts/service.ts` (D-Q — Phase 06 ADDS `listFindersWithLockedCommissions` + `generateCsv` + admin UI to THIS service): `createPayoutBatch` (reserve: stamps paid_payout_id, stays locked; finder cpf/pix missing → `finder_payout_details_missing` 422 not crash), `markPayoutPaid` (ONLY locked→paid flip), `getPayoutsAdmin`, `getPayoutsByFinder`. `payoutsRouter`/`payoutsAdminRouter`. **NO payout_batches/payout_batch_id/in_payout** — Phase 06 T02 must NOT create them.
- `apps/api/src/domains/audit/service.ts`: `canonicalJson`, `computeEntryHash`, `verifyChain`, `writeAuditEntry(db, {actorUserId, action, entityType, entityId, before/afterJsonb})` — **all phases' audit writes route through this now**. `AuditActionSchema` enum. `auditRouter` (GET paginated + GET /verify-chain). **prev_hash is the PREPENDED sha256 arg, NOT inside canonical_json (D8).** Pre-Phase-05 '' placeholder rows are OUTSIDE the chain; new rows chain from genesis; checks filter to 64-char-hash rows.
- `apps/api/src/domains/sellers/clerk-webhook.ts`: `clerkWebhookRouter` (svix verify user.created → backfill sellers.clerk_user_id by email via getAdminDb). Mounted `POST /api/v1/webhooks/clerk`. `CLERK_WEBHOOK_SIGNING_SECRET` in env.ts + both .env examples.
- `apps/api/src/jobs/nightly-job.ts`: `setupNightlyJob` (node-cron `0 3 * * *`, single instance), `runHoldPromotion`, `stopNightlyJob`. Promotes pending→locked (D-K). **Phase 06 must register any payout job on THIS scheduler, not a second one.**
- `apps/web` admin: ConversionsPage/CommissionsPage/AuditLogPage + hooks + CommissionStateBadge + api-client `adminConversionsApi`/`adminCommissionsApi`/`adminAuditApi`. Routes /admin/conversions, /admin/commissions, /admin/audit + AdminNav. i18n in BOTH pt-BR+en. **Admin payouts UI is the only Phase 05 UI deferred to Phase 06 (D-Q).**

**Deviations (carry forward):**
1. **ingestConversion on getAdminDb() not getDb()** (overrides plan T05 text; honors D-C cross-tenant rule). Required `GRANT SELECT ON clicks, referral_links TO fxl_finders_admin` in migration 0004 (BYPASSRLS bypasses RLS policies, NOT table GRANTs).
2. Audit chain: prev_hash PREPENDED to the hash, not a canonical_json field (D8 literal).
3. Added a Critical-fix guard: `POST /api/v1/conversions` rejects `event_type !== 'sale'` with 422 `unsupported_event_type` (a refund on the sale path would mis-book a positive commission). Refunds flow via `POST /api/v1/conversions/refund`.
4. Known v1.0 gap (Info): refund-via-webhook has no idempotency dedupe for already-paid rows (deferred v1.1).
